  ...
  <style>{`
    ...
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
    ...
    .nav-hamburger {
      ...
      color:${C.greenDark}; padding:6px; border-radius:8px;
      transition:background .2s;
    }
    .nav-hamburger:hover { background:${C.greenMist}; }
    ...
    .mobile-menu {
      ...
      background:${C.warmWhite}; padding:20px 24px 32px;
    ...
    .mobile-menu-close {
      ...
      background:${C.greenMist}; border:none; display:flex; align-items:center;
      justify-content:center; cursor:pointer; color:${C.greenDark}; margin-bottom:8px;
      transition:background .2s;
    }
    .mobile-menu-close:hover { background:${C.greenPale}; }
    .mobile-menu-item {
      ...
      color:${C.textMuted}; transition:background .2s, color .2s;
    }
    .mobile-menu-item:hover { background:${C.greenMist}; color:${C.greenDark}; }
    .mobile-menu-item.active { background:${C.greenMist}; color:${C.greenDark}; }
...
export default function App() {
  ...
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false); // "envios" | "devoluciones" | "datos"
  ...
  const [pedidoData,  setPedidoData]    = useState(null);   // { cliente:{}, items:[] }
  ...
  /* Busca el último pedido por celular en Google Sheets */
  const buscarUltimoPedido = useCallback(async () => {
    const phone = pedidoPhone.trim().replace(/\s+/g, "");
    if (!phone || phone.length < 7) {
      setPedidoError("Ingresa un número de celular válido.");
      return;
    }
    ...
    try {
      ...
      if (!ct.includes("application/json"))
        throw new Error("El servidor no devolvió JSON. Verifica el Apps Script.");
      const data = await res.json();
      if (data?.ok === false || !data?.cliente)
        throw new Error(data?.error || "No encontramos pedidos con ese número.");
      setPedidoData(data);
    } catch (e) {
  ...
  const cargarPedidoAlCarrito = useCallback(() => {
    ...
    pedidoData.items.forEach((item) => {
      ...
      if (found) {
        ...
        const variant = variants.find(
          (v) => normalize(v.presentation) === normalize(item.presentacion || "")
        ) || variants[0];
        ...
        // Producto no está en catálogo actual, lo agrega como-es
        addToCart({
    ...
    toast("✓ Productos del pedido agregados al carrito");
    goTo("catalogo");
    setDrawerOpen(true);
    ...
    <div className="soin-root">
      ...
      <nav className="nav">
        ...
        <div className="nav-links">
          ...
          <button className="nav-hamburger tap" onClick={() => setMobileMenuOpen(v => !v)} aria-label="Abrir menú">
            <Menu size={22} />
          </button>
        </div>
      </nav>
      ...
      <footer className="footer">
        ...
        <div className="footer-grid">
          ...
          <div>
            ...
            <p className="footer-tagline">Todo lo que tu mascota necesita, en un solo lugar. Productos naturales con respaldo veterinario.</p>
